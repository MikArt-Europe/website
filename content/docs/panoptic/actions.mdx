...
events:
  - conditions:
    ...
    actions:
      - type: send_message
        value: "&6Lucky find! You mined %block_type% at %block_location%"
      - type: run_command_as_console
...
### send_message
Sends a message directly to the player who triggered the event. Supports color codes and formatting.

```yaml
- type: send_message
  value: "&aWelcome back, %player%! &7You're in world: &e%world%"
```
...
events:
  - conditions:
    ...
    actions:
      - type: send_message
        value: "&cOops! Teleporting you to safety..."
      - type: run_command_as_player
        value: "spawn"
```
...
events:
  - conditions:
    ...
    actions:
      - type: send_message
        value: "&6Welcome to our server! Here's a starter kit!"
      - type: run_command_as_console
...
events:
  - conditions:
    ...
    actions:
      - type: send_message
        value: "&cYou cannot break spawners in the survival world!"
      - type: cancel
```

### placeholder_message
Sends a message using PlaceholderAPI placeholders. Requires PlaceholderAPI to be installed.
*Developer note: It's not completely clear why it was chosen to be it's own action instead of using the `send_message` action with placeholders directly.*

```yaml
- type: placeholder_message
  value: "&7Your balance: &a$%vault_eco_balance%"
```

### miniplaceholder_message
Similar to placeholder_message but uses MiniPlaceholders instead. Requires MiniPlaceholders to be installed.
*Developer note: It's not completely clear why it was chosen to be it's own action instead of using the `send_message` action with placeholders directly.*

```yaml
- type: miniplaceholder_message
  value: "&7Server TPS: &e%server_tps%"
```
...
events:
  - conditions:
    ...
    actions:
      - type: send_message
        value: "&6&l✦ RARE DROP! &r&eYou found a hidden treasure!"
      - type: run_command_as_console
...
events:
  ...
  - conditions:
    ...
    actions:
      - type: send_message
        value: "&6Good afternoon, %player%! 🌤️"
  - conditions:
      - type: placeholder
        value: "%server_time_H%>=18"
    actions:
      - type: send_message
        value: "&9Good evening, %player%! 🌙"
```

## Tips and Tricks

- **Color Codes**: Use `&` followed by a color/format code (like `&a` for green or `&l` for bold)
...
**Note**: The `placeholder_message` and `miniplaceholder_message` actions might be merged in a future update since they serve similar purposes.
