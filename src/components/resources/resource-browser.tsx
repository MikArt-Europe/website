...
export function ResourceBrowser({ resources }: { resources: Resource[] }) {
    ...
    useEffect(() => {
        async function fetchPlatforms() {
            ...
            const latestVersionIds = resources.map((r: Resource) =>
                r.versions.sort((a: Version, b: Version) =>
    ...
    return (
        <div className="container mx-auto py-8">
            <div className="mb-8 bg-gradient-to-r from-primary/10 to-secondary/10 p-8 rounded-xl">
                <h1 className="text-4xl font-bold mb-3 bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Minecraft Plugins</h1>
                <p className="text-muted-foreground mb-6">Browse our collection of high-quality plugins to enhance your Minecraft server</p>
            ...
            {filteredResources.length > 0 ? (
                    ...
                    {view === 'grid' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredResources.map((resource: Resource) => (
                                <Card key={resource.id} className="flex flex-col h-full overflow-hidden group hover:shadow-md transition-all duration-300 border-primary/10 hover:border-primary/30 relative">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        <div className="space-y-4">
                            {filteredResources.map((resource: Resource) => (
                                <Card key={resource.id} className="overflow-hidden group hover:shadow-md transition-all duration-300 border-primary/10 hover:border-primary/30 relative">
                                    <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-secondary/5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
                ...
                <div className="text-center py-16 bg-muted/20 rounded-lg border border-border">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mx-auto text-muted-foreground mb-4">
